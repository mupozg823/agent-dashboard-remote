// ── renderer-core.ts ── Character drawing, background, particles, active screens
...
export function cH(): number { return S.pixiApp ? S.pixiApp.screen.height : ((document.getElementById('c') as HTMLCanvasElement).height / S.dpr); }

// ── PixiJS v8 init ──
export async function initPixi(): Promise<void> {
  const container = document.querySelector('.scene') as HTMLElement | null;
  ...
  try {
    ...
    (['bg', 'weather', 'desks', 'agents', 'particles', 'hud', 'effects'] as const).forEach(name => {
...
function initCanvasFallback(): void {
  S.pixiReady = false;
  ...
  (window as unknown as Record<string, unknown>)._mainCx = mainCx;
}
...
export function drawCh(px: number, py: number, type: string, wf: number, dir: number, work: boolean, bub: string, ag: AgentInstance | null): void {
  ...
  if (work) {
    ...
    if (ag && ag.wt > 60) { const sw = ((fr * 2) % 25); cx.globalAlpha = 1 - sw / 25; cx.fillStyle = '#88CCFF'; cx.fillRect(px + 5 * s, y - 5 * s + sw * .5, s * .5, s * .8); cx.globalAlpha = 1; }
...
function drawWorkerBadge(px: number, py: number, ag: AgentInstance | null, s: number): void {
...
function drawNameTag(px: number, y: number, c: { l: string; p: string }): void {
...
function drawWorkFx(px: number, y: number, type: string, s: number): void {
  const cx = S.cx!, fr = S.fr;
...
function drawStatusIcon(x: number, y: number, type: string): void {
  const cx = S.cx!, fr = S.fr;
...
function drawBub(x: number, y: number, t: string): void {
  const cx = S.cx!;
  ...
  const bubCx = bx2 + w / 2;
  if (twoLine) {
    cx.fillStyle = '#999999'; cx.font = 'bold 9px sans-serif'; cx.fillText(line1, bubCx, by + 10);
    cx.fillStyle = '#5A4020'; cx.font = 'bold 11px monospace';
    let d2 = line2; while (cx.measureText(d2).width > textMaxW && d2.length > 3) d2 = d2.slice(0, -1);
    cx.fillText(d2, bubCx, by + 22);
  } else {
    cx.fillStyle = '#8B6F47'; let fl = full; while (cx.measureText(fl).width > textMaxW && fl.length > 3) fl = fl.slice(0, -1);
    cx.fillText(fl, bubCx, y - h / 2);
  }
  cx.textBaseline = 'alphabetic';
}

// ── Background cache ──
export function buildBg(w: number, h: number): void {
  if (w < 10 || h < 10) return;
...
export function drawActiveScreen(x: number, fy: number, agentType: string): void {
...
export function getParticleTexture(color: string, shape: string, size: number): PIXI.Texture {
...
// ── Particles ──
export function spawnP(x: number, y: number, n: number, type?: string, tool?: string): void {
  ...
  for (let i = 0; i < Math.min(n, 12); i++) {
    ...
    const p = { x: x + (Math.random() - .5) * 14, y: y + (Math.random() - .5) * 6, vx: (Math.random() - .5) * 4.5, vy: -Math.random() * 4.5 - 1, l: 35 + Math.random() * 25, c: color, z: sz, shape, rot: Math.random() * 6.28, rv: (Math.random() - .5) * .25, sprite: null as PIXI.Sprite | null };
    if (S.pixiReady && S.L.particles) {
      try { const tex = getParticleTexture(color, shape, Math.ceil(sz)); const sp = new PIXI.Sprite(tex); sp.anchor.set(0.5); sp.x = p.x; sp.y = p.y; sp.rotation = p.rot; S.L.particles.addChild(sp); p.sprite = sp; } catch (e) { }
    }
    S.pts.push(p);
  }
}

export function drawPts(): void {
  const cx = S.cx!;
...
// ── Floating text ──
export function spawnFloatingText(x: number, y: number, text: string, color?: string, size?: number): void {
  if (S.floatingTexts.length > 20) return;
  const ft = { x, y, text, color: color || '#FFD080', size: size || 12, life: 60, vy: -1.2, alpha: 1, sprite: null as PIXI.Text | null };
  if (S.pixiReady && S.L.effects) {
    try {
      const t = new PIXI.Text({ text, style: { fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', fontWeight: 'bold', fontSize: ft.size, fill: ft.color, dropShadow: true, dropShadowColor: '#000000', dropShadowDistance: 1, dropShadowAlpha: 0.6 } });
      t.anchor.set(0.5); t.x = x; t.y = y; S.L.effects!.addChild(t); ft.sprite = t;
    } catch (e) { }
  }
...
// ── Shake ──
export function triggerShake(intensity?: number): void { S.shakeFrames = 12; S.shakeIntensity = intensity || 3; }
