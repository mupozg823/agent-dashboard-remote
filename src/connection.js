...
import { S, DESKS, AT, NR, TOOL_COLORS, SSE_PORTS, WORKER_ROLES } from './state.js';
...
import { trackToolCall, trackMcp } from './game-systems.js';
...
export function connectWith(url, key, sessionId) {
  ...
  S.channel.on('broadcast', { event: 'orch-dag' }, ({ payload }) => { S.orchRun = S.orchRun || {}; S.orchRun.dag = payload.dag; S.orchRun.total = payload.steps; S.orchRun.done = 0; S.workers = {}; S.activeWorkerAgents = {}; if (Array.isArray(payload.dag)) { payload.dag.forEach(st => { if (st.worker) { S.workers[st.id] = { role: st.worker, status: 'pending', stepName: st.name }; const wr = WORKER_ROLES[st.worker]; if (wr) S.activeWorkerAgents[wr.agentType] = st.worker; } }); } toast('DAG \uC0DD\uC131: ' + payload.steps + '\uAC1C \uC2A4\uD15D', 'ok'); narr('\uD0DC\uC2A4\uD06C \uBD84\uD574 \uC644\uB8CC: ' + payload.steps + '\uAC1C \uC2A4\uD15D', 'agent'); for (let i = 0; i < Math.min(payload.steps, 12); i++) spawnP(cW() * Math.random(), cH() * Math.random(), 3, 'success'); updateOrchUI(); });
  S.channel.on('broadcast', { event: 'orch-step-start' }, ({ payload }) => { if (payload.worker && payload.step) { S.workers[payload.step] = { role: payload.worker, status: 'running', stepName: payload.name }; const wr = WORKER_ROLES[payload.worker]; if (wr) S.activeWorkerAgents[wr.agentType] = payload.worker; } toast('\uC2E4\uD589: ' + payload.name, 'in'); narr(payload.step + ': ' + payload.name + ' (' + payload.executor + ')', 'bash'); updateOrchUI(); });
  S.channel.on('broadcast', { event: 'orch-step-done' }, ({ payload }) => { if (payload.step && S.workers[payload.step]) { S.workers[payload.step].status = payload.success ? 'completed' : 'failed'; } if (payload.success) { spawnP(cW() / 2, cH() * .4, 6, 'success'); } else { triggerShake(3); } updateOrchUI(); });
  S.channel.on('broadcast', { event: 'orch-progress' }, ({ payload }) => { if (S.orchRun) S.orchRun.done = payload.done; if (payload.workerStats) S.workerStats = payload.workerStats; updateOrchUI(); });
  S.channel.on('broadcast', { event: 'orch-complete' }, ({ payload }) => { S.orchRun = payload; const ok = payload.status === 'done'; if (Array.isArray(payload.dag)) { payload.dag.forEach(st => { if (st.worker && st.id) S.workers[st.id] = { role: st.worker, status: st.status || 'completed', stepName: st.name }; }); } toast(ok ? '\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \uC644\uB8CC!' : '\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \uC2E4\uD328', ok ? 'ok' : 'er'); narr('\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 ' + payload.status + ': ' + payload.steps?.completed + '/' + payload.steps?.total, 'agent'); if (ok) { for (let i = 0; i < 15; i++) spawnP(cW() * Math.random(), cH() * Math.random(), 4, 'success'); } else triggerShake(6); S.activeWorkerAgents = null; updateOrchUI(); });
...
export function onE(e) {
  ...
  // Build real detail for speech bubble: cmd or last path segments
  const detail = e.cmd || e.path?.split(/[/\\]/).slice(-2).join('/') || e.summary || '';
  const ds = detail || desc(e);
  ...
  if (e.decision === 'deny') { narr(pick(NR.deny), 'agent'); spawnP(cW() / 2, cH() * .3, 8, 'error'); triggerShake(4); toast('\uCC28\uB2E8: ' + (e.cmd || e.tool || '').slice(0, 30), 'er'); }
  else if (e.err) { spawnP(deskX, cH() * .5, 5, 'error'); const p = NR[e.tool] || NR.idle; narr(pick(p), AT[ai]); }
  else { const p = NR[e.tool] || NR.idle; narr(pick(p), AT[ai]);
